...
interface ProjectsContextType {
  ...
  updateProjectProgress: (id: string, progressAmount: number) => void
  addJournalEntry: (projectId: string, content: string) => void
  getStreak: () => number
  getMonthlyData: () => any[]
...
export function ProjectsProvider({ children }: { children: ReactNode }) {
  ...
  // Actualizar la función addProject para usar el nuevo formato de hitos
  const addProject = (
    projectData: Omit<
      Project,
      "id" | "totalTarget" | "progressPercentage" | "milestones" | "progressHistory" | "lastUpdated"
    ...
    try {
      ...
      const newProject: Project = {
        ...
        totalTarget,
        progressPercentage,
        milestones: calculateMilestones(projectData.startDate, projectData.targetDate),
        progressHistory: [],
        lastUpdated: today,
      }
  ...
  const updateProject = (updatedProject: Project) => {
    // Check if it's in active projects
    if (projects.some((p) => p.id === updatedProject.id)) {
      setProjects((prev) => prev.map((p) => (p.id === updatedProject.id ? updatedProject : p)))
    }
    // Check if it's in completed projects
  ...
  const deleteProject = (id: string) => {
    ...
    // También eliminar las entradas de diario asociadas
    setJournalEntries((prev) => prev.filter((entry) => entry.projectId !== id))

    // Mostrar notificación (opcional, si tienes un sistema de notificaciones)
    try {
      if (typeof window !== "undefined") {
        // Usar el sistema de notificaciones del navegador si está disponible
  ...
  // Modificar la función updateProjectProgress para actualizar los hitos cuando se alcancen
  const updateProjectProgress = (id: string, progressAmount: number) => {
    const today = new Date().toISOString().split("T")[0]
  ...
  const addJournalEntry = (projectId: string, content: string) => {
    const today = new Date().toISOString().split("T")[0]
